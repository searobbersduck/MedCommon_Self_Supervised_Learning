...
import sys
root = os.path.abspath(os.path.join(os.path.abspath(os.path.dirname(__file__)), os.path.pardir))
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for name in model_names:
        if args.arch.startswith(name):
            arch = name
            sub_arch = int(args.arch.replace(arch, ''))
            backbone = models3D.__dict__['resnet'].generate_model(sub_arch, n_input_channels=1, widen_factor=0.5, n_classes=args.moco_dim)
